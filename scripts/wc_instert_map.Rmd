...
st <- st_read('./data/IOPAC_SHP/USA_1_v41.shp') %>% filter(NAME_1 %in% c('Oregon', 'California', 'Washington'))
  # here('data', 'IOPAC_SHP','USA_1_v41.shp')) 
lt <- read_csv('./Data/iopac_lat-long-r.csv')
...
shp <- readRDS('./Data/IOPAC_shapefiles.RDS') %>% filter(IOPAC_PORT_GROUP != 'Columbia River')
# # read in iopac data
iopac <- read_csv('./Data/iopac_lat-long-mod.csv') %>%
  mutate(PRIMARY_LATITUDE = case_when(IOPAC_PORT_GROUP == 'Brookings' ~ PRIMARY_LATITUDE + 0.5,
...
iopac_lt <- read_csv('./Data/iopac_lt.csv')
# iopac_sh
