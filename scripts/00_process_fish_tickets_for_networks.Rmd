...
## check proportion of all tickets and revenue with council == "N" and what ports they're landed at
all_revenue <- sum(rawdat.sub$afi_revenue, na.rm = TRUE)
rawdat.sub %>% 
  left_join(
  ...
  summarize(total_revenue = sum(afi_revenue, na.rm = TRUE),
            pct_total_revenue = total_revenue/all_revenue*100) %>% 
  filter(council == "N")
