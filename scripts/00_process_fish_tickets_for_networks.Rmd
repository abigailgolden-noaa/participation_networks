...
Remove fish tickets with an unknown (`UNKNOWN`) OR MISSING (`""` / `MISSING`) vessel identifier and vessels where `COUNCIL_CODE` is not `P`. This filters out fish tickets for fish landed in AK and sold on the WC (`council == "N"`) and tribal vessels, where many vessels do not have IDs and are assigned random numbers beginning `ZZ` (`council == "*"`). Recommended by Erin Steiner 12/02/2024. As of December 2024, this results in dropping about 2.6M records. About 6M records remain
```{r filter}
...
Remove non-crab and non-shrimp shellfish (`SPGRPN2 == 65`) per Erin Steiner's recommendation on 12/2/24. She says that aquaculture is sporadically recorded under that species grouping and fish tickets are also created for some other non-commercial sources of revenue, like shellfish growers selling oyster seeds, and these sources of revenue should be excluded from commercial revenue calculations.
...
cat("Added in ", sum(!is.na(rawdat.sub5$adj_afi_ppp)) - sum(!is.na(rawdat.sub5$afi_ppp)), " adjusted ppp.") # this is the modified check Jameal put in
